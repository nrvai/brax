...
def reccurent_actor_step(
  ...
  """Collect data."""
  actions, policy_extras = policy(
      env_state.obs, env_state.priv, env_state.hidden_state, key
  )
  nstate = env.step(env_state, actions)
  nstate = nstate.replace(hidden_state=policy_extras['hidden_state'])
  state_extras = {x: nstate.info[x] for x in extra_fields}
  return nstate, Transition(  # pytype: disable=wrong-arg-types  # jax-ndarray
      observation=env_state.obs,
      priv=env_state.priv,
      hidden_state=nstate.hidden_state,
      action=actions,
...
def reccurent_generate_unroll(
  ...
  def f(carry, unused_t):
    ...
    nstate, transition = reccurent_actor_step(env, state, policy, current_key, extra_fields=extra_fields)
...
class Evaluator:
    ...
    def __init__(self,
        ...
        def generate_eval_unroll(policy_params: PolicyParams,
            ...
            eval_first_state.info["curriculum"] = jnp.full(num_eval_envs, 4.0)
